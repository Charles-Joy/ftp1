...
int main(int argc,char* argv[])
{
	...
	while(1)
	{
		...
		ret=epoll_wait(epfd,evs,num+2,-1);//等待注册的事件发生
		for(i=0;i<ret;i++)
		{
			printf("事件发生，ret=%d\n",ret);
			if(sfd==evs[i].data.fd)
			{
				...
				for(j=0;j<num;j++)
				{
					if(0==p[j].busy)
					{
						send_fd(p[j].fd,new_fd,0);//发送描述符给子进程
						break;
					}
	        	}
				p[j].busy=1;//把子进程置为忙碌状态
				printf("找到对应子进程，描述符发送成功。p[j].pid=%d is busy\n",p[j].pid);
				close(new_fd);//父进程关闭new_fd,引用计数new_fd
			}
			...
			for(j=0;j<num;j++)//发现对应子进程的描述符可读，设置对应的子进程为非忙碌
			{
				if(p[j].fd==evs[i].data.fd)
				{
					...
					printf("对应子进程退出，p[j].pid=%d is not busy,数据读取成功，ret1=%d\n",p[j].pid,ret1);
					p[j].busy=0;
				}
			}
		}
